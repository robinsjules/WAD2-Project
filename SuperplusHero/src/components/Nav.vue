  ...
  <nav v-if="showNavBar" class="navbar navbar-expand-md navbar-dark bg-dark navcolor">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <!-- Modal trigger -->
          <a class="nav-link" href="#" data-bs-toggle="modal" data-bs-target="#locationModal">
        ...
        <li class="nav-item">
          <router-link to="/"> <!-- Edit to Cart Page, also apply to index.js router -->
            <a class="nav-link" href="#" data-bs-toggle="modal" data-bs-target="#cartModal" @click="checkCart()">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <input id="autocomplete" class="form-control" type="text" placeholder="Enter a location" v-model="location">
          <p class="text-help mt-2">- OR -</p>
          <button class="btn mt-2 btn-success color-coord" @click="useCurrentLocation">
        ...
        <div class="modal-body">
          ...
          <div v-for="(item, idx) in cart" :key="idx" class="cart-item">
            ...
            <div class="cart-item-details">
              ...
              <span class="price"> Price: $<s>{{ item.OriginalPrice }}</s><strong class="ms-2 text-danger"> ${{
                item.SalePrice }}</strong></span>

              <div class="cartItemQuantity">
                Quantity:
                <button class="btn btn-success color-coord" @click="decreaseQuantity(item)"
                  :disabled="desiredQuantity[item.id] <= 1">-</button>
                {{ desiredQuantity[item.id] || 1 }}
                <!-- If item id exists in desiered quantity object set value to 1 if not go next -->
                <button class="btn btn-success color-coord" @click="increaseQuantity(item)"
                  :disabled="desiredQuantity[item.id] >= item.Quantity">+</button>
  ...
  methods: {
    ...
    checkedOut() {
      ...
      if (Cookies.get("showCheckoutAlert")) {
        if (Cookies.get("showCheckoutAlert") == 'true') {
